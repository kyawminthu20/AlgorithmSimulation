...
void RR_Simulation(int maxTime){
  ...
  for(int k=0; k <= maxTime; k++)
  {
    ...
    char *nextProcessToRun;
    nextProcessToRun = (char*) malloc(sizeof(char)*(2+1));
    char *processAtCPU;
    processAtCPU = (char*) malloc(sizeof(char)*(2+1));

    //Check If any process running in the CPU
    bool occupiedCPU = false;
    for(int o = 0; o < 8; o++)
    {
      if(processList[o].processState == Running)
      {
        strcpy(processAtCPU, processList[o].processID);
        occupiedCPU = true;
      }   
    }
    //Reduce Running time from Running process
    int currentRunningProcess = 1000;
    for(int j = 0; j < 8; j++)
    {
      if (processList[j].processState == Running){
        currentRunningProcess = j;
        processList[j].runTime = processList[j].runTime + 1;
        processList[j].remainingTime = processList[j].remainingTime -1;
        quantumCounter = quantumCounter + 1;
    ...
    if(newProcessReady && (!occupiedCPU))
      contextSwitch = true;
    ...
    if(contextSwitch == true)
    {
      ...
      for(int m = 0; m< 8; m++)
      {
        if(strcmp(processList[m].processID, nextProcessToRun))
        {
